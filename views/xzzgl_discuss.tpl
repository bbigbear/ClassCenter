...
<form class="layui-form layui-form-pane1" action="">
  ...
  <div class="layui-inline">
    <label class="layui-form-label">协作组名称</label>
    <div class="layui-input-inline" style="width: 150px;">
      <input type="text" id="GroupName" autocomplete="off" class="layui-input">
    </div>
  </div>
  </div>
  <div class="layui-form-item">
    <div class="layui-inline layui-layout-right" style="padding:10px;">
		...
		<button class="layui-btn" id="add1">进入讨论</button>
	...
	$(function(){
	 ...
	 //table 渲染
	  table.render({
	    elem: '#list'
	    ,height: 315
	    ,url: '/v1/xzz/getdata'//数据接口
	    //,page: true //开启分页
		,id: 'listReload'
	    ,cols: [[   
		  {field:'Name',  title:'协作组名称', width:120}
	      ,{field:'GroupLeader',  title:'组长', width:120}
		  ,{field:'GroupMember',  title:'备注', width:250}
		...
		table.on('tool(announcement)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
		    ...
		    if(layEvent === 'edit'){
			 ...
			 // offset: 'rb', //右下角弹出
			  ...
			  maxmin: true,
			  anim: 2,
			  content: ['/v1/jsbk/group/edit?id='+data.Id], //iframe的url，no代表不显示滚动条
		      ...
		      layer.confirm('真的删除行么', function(index){
				...
				$.post('/v1/xzz/del', jsData, function (out) {
	                if (out.code == 200) {
	...
	$('#add').on('click',function(){
		layer.open({
			 ...
			 // offset: 'rb', //右下角弹出
			  ...
			  maxmin: true,
			  anim: 2,
			  content: ['/v1/xzz/add'], //iframe的url，no代表不显示滚动条
		});
		return false;
	});
